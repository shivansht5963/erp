...
class FeeReminderAdmin(admin.ModelAdmin):
    list_display = ('student_info', 'subject', 'reminder_date', 'sent_status', 'sent_date')
    list_filter = ('sent', 'reminder_date')
    search_fields = ('student__user__first_name', 'student__user__last_name', 'student__roll_number', 'subject')
    readonly_fields = ('created_at', 'sent_date', 'created_by')
    ...
    def send_reminders(self, request, queryset):
        ...
        if reminders_sent > 0:
            self.message_user(request, f"Successfully sent {reminders_sent} reminder(s).")
        else:
            self.message_user(request, "No reminders were sent. They may have already been sent.", level=messages.WARNING)
